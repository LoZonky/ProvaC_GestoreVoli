...
public class GestorePrenotazioneVoli {
	...
	/*
	 * Per la lettura da file devi: 
	 * 1) Avere una stringa che ti dica dove andare a prendere il dato 
	 * 		String path = "nome_file.txt";
	 * 
	 * 2) Impostare la variabile di inputManager 
	 * 		FileInputManager in = new FileInputManager(path);
	 * 
	 * 3) avere una variabile stringa che rappresenti le righe e ciclare in un while finchè non diventa vuota (NULL)
	 * 		String righe;
	 * 		do{
	 * 			righe = in.readLine();  // Così ottengo la prima riga
	 * 		
	 * 			...
	 * 		}while(righe!=null)
	 *  
	 *  4) Avere una variabile che rappresenta i campi della riga separati da un simbolo particolare
	 *  	String campi[];
	 *  	campi = righe.split(";"); // In questo caso l'elemento separatore è ";"	
	 */
	public void caricaDaFile(String path) throws VoloPienoException {
		String righe, campi[];
		FileInputManager in = new FileInputManager(path);
		do {
			righe = in.readLine();
	 ...
	 *            numero di posti
	 */
	public void inserisciVolo(String o, String de, Data da, int n, int ID) {
		voli.add(new Volo(o, de, da, n, ID));

	}
	public void inserisciVolo(String o, String de, Data da, int n) {
	 ...
	 */
	public void inserisciPrenotazione(Prenotazione p, int id) throws VoloPienoException {
		for (Volo v : voli) {
			if (v.getID() == id) {
				if (v.pieno())
	 ...
	 * @throws VoloPienoException
	 */
	public void sposta(String idpren, int idvoloNuovo) throws VoloPienoException {
		Volo src = null;
